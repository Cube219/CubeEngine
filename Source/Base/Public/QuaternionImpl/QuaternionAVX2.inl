...
namespace cube
{
    ...
    inline Quaternion Quaternion::FromAxisAngle(const Vector3& axis, float angle)
    {
        ...
        Quaternion res;
        res.mData = _mm_fmadd_ps(a.mData, _mm_set1_ps(s), scalar);
    ...
    inline Quaternion& Quaternion::operator*=(const Quaternion& rhs)
    {
        ...
        res = _mm_fmadd_ps(x1, _mm_mul_ps(q2_1, pnpn), res);
        res = _mm_fmadd_ps(y1, _mm_mul_ps(q2_2, ppnn), res);
        res = _mm_fmadd_ps(z1, _mm_mul_ps(q2_3, nppn), res);
    ...
    inline Matrix Quaternion::ToRotationMatrix() const
    {
        ...
        */
        ...
        Vector4 row0;
        row0.mData = _mm_fmadd_ps(yyyy, _mm_mul_ps(npnp, yxwz), _mm_mul_ps(zzzz, _mm_mul_ps(nppn, zwxy)));
        // (xy-zw, -xx-zz, xw+zy, -xz+zx(->0))
        Vector4 row1;
        row1.mData = _mm_fmadd_ps(xxxx, _mm_mul_ps(pnpn, yxwz), _mm_mul_ps(zzzz, _mm_mul_ps(nnpp, wzyx)));
        // (xz+yw, -xw+yz, -xx-yy, xy-yx(->0))
        Vector4 row2;
        row2.mData = _mm_fmadd_ps(xxxx, _mm_mul_ps(pnnp, zwxy), _mm_mul_ps(yyyy, _mm_mul_ps(ppnn, wzyx)));
