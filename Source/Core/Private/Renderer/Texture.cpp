...
namespace cube
{
    TextureResource::TextureResource(const TextureResourceCreateInfo& createInfo)
    {
        ...
        TextureHelper::ValidateInfo(info);
    ...
    void TextureHelper::ValidateInfo(const gapi::TextureInfo& info)
    {
        // TODO: Check it is correct.
        switch (info.type)
        {
        case gapi::TextureType::Texture1D:
            CHECK_FORMAT(info.height == 1, "Height must be 1 in Texture1D type.");
            CHECK_FORMAT(info.depth == 1, "Depth must be 1 in Texture1D type.");
            CHECK_FORMAT(info.arraySize == 1, "Array size must be 1 in Texture1D type.");
            break;
        case gapi::TextureType::Texture1DArray:
            CHECK_FORMAT(info.height == 1, "Height must be 1 in Texture1DArray type.");
            CHECK_FORMAT(info.depth == 1, "Depth must be 1 in Texture1DArray type.");
            break;
        case gapi::TextureType::Texture2D:
            CHECK_FORMAT(info.depth == 1, "Depth must be 1 in Texture2D type.");
            CHECK_FORMAT(info.arraySize == 1, "Array size must be 1 in Texture2D type.");
            break;
        case gapi::TextureType::Texture2DArray:
            CHECK_FORMAT(info.depth == 1, "Depth must be 1 in Texture2DArray type.");
            break;
        case gapi::TextureType::Texture3D:
            CHECK_FORMAT(info.arraySize == 1, "Array size must be 1 in Texture3D type.");
            break;
        case gapi::TextureType::TextureCube:
            CHECK_FORMAT(info.depth == 1, "Depth must be 1 in TextureCube type.");
            CHECK_FORMAT(info.arraySize == 1, "Array size must be 1 in TextureCube type.");
            break;
        case gapi::TextureType::TextureCubeArray:
            CHECK_FORMAT(info.depth == 1, "Depth must be 1 in TextureCubeArray type.");
            break;
        default:
            NOT_IMPLEMENTED();
            break;
